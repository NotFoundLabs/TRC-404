...
(async () => {
  ...
  let { wallet_contract: user3_wallet, secretKey: user3_secretKey } = await getWalletContract(user3);
  ...
  await sleep(35000);
   ...
   //3.2 invoke transfer FT function  of Trc404-wallet  ,user 2 transfer ft to user1
    let gas_fee = 0.18;  
   let trasfer_ft_TxSeqo=await invokeTransferFTFromTrc404Wallet(0,user1_wallet,user1_secretKey,compliedCodes,
  ...
  // //**********4.invoke contract (test transfer NFT)*******************/
   //  invoke transfer NFT function  of Trc404-userNft contract , transfer NFT from user1 --> user2
